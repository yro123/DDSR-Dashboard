...
const btn = {
  base:   { border: 'none', borderRadius: 6, padding: '4px 11px', fontSize: 12, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' } as CSSProperties,
  save:   { background: 'var(--accent)', color: 'var(--accent-text)' } as CSSProperties,
  cancel: { background: 'var(--surface-2)', color: 'var(--text-muted)' } as CSSProperties,
  danger: { background: '#fee2e2', color: '#b91c1c' } as CSSProperties,
  ghost:  { background: 'transparent', color: 'var(--accent)', padding: '3px 8px' } as CSSProperties,
}
...
const Btn = ({ style, children, ...props }: BtnProps) => (
...
function SortableItemRow({ item, hasColor, isColorPalette, onReload }: SortableItemRowProps) {
  const { api } = useProject()
  const { setNodeRef, attributes, listeners, transform, transition, isDragging } = useSortable({ id: item.id })
  const [editing, setEditing] = useState(false)
  const [form, setForm] = useState<{ value: string; color: string }>({ value: item.value, color: item.color || '' })
  const set = (k: 'value' | 'color', v: string) => setForm(f => ({ ...f, [k]: v }))
...
function AddItemForm({ category, projectId, hasColor, isColorPalette, onReload, nextOrder }: AddItemFormProps) {
  const { api } = useProject()
  const [form, setForm] = useState<{ value: string; color: string }>({ value: '', color: '#3B82F6' })
...
interface CategorySectionProps {
  meta: CategoryMeta
  items: AppConfigRow[]
  globalItems: AppConfigRow[]
  projectId: number | null
  isOverride: boolean
  onReload: () => void
}

function CategorySection({ meta, items, globalItems, projectId, isOverride, onReload }: CategorySectionProps) {
  const { api } = useProject()
...
// Shape of the /api/config response: per-category arrays plus project metadata.
interface ConfigData {
  project_id?: number | null
  global?: Record<string, AppConfigRow[]>
  is_project_override?: Record<string, boolean>
  [category: string]: unknown
}

export default function ConfigTab({ projectSlug: propProjectSlug }: ConfigTabProps) {
  ...
  const reload = useCallback(() => {
    ...
    api.get<ConfigData>(url).then(setConfigData).catch(() => {})
  ...
  const projectId = scope === 'project' ? (configData.project_id ?? null) : null
  ...
  return (
    <div>
      ...
      {CATEGORY_META.map(meta => {
        const items = ((configData[meta.key] as AppConfigRow[]) || []).slice().sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0))
        const global = (globalData[meta.key] || []).slice().sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0))
