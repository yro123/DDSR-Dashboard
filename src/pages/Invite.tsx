...
/** Minimal shape of the GET /api/invitations/:token response we consume. */
...
export default function Invite() {
        ...
        authClient.getSession({ fetchOptions: { cache: 'no-store' } })
          .then(async () => {
            try {
              ...
              const accessible = await res.json() as ClientWithProjects[]
  ...
  const handlePassword = async (e: FormEvent) => {
    ...
    try {
      ...
      // better-auth errors surface a `message`; widen to any for access.
  ...
  const tabBtn = (label: string, active: boolean, onClick: () => void): ReactNode => (
