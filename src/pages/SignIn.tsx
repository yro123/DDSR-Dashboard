...
function Field({ label, type = 'text', value, onChange, placeholder, autoFocus }: FieldProps) {
  return (
...
// better-auth client methods return a `{ error }` shape; we read message defensively.
interface AuthError {
  message?: string
...
export default function SignIn() {
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const e = err as AuthError
      setError(e?.message || e?.error?.message || 'Failed to send link. Try again.')
    }
    setBusy(false)
  }
