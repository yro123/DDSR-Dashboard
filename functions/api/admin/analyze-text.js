...
export async function onRequestPost({ env, request }) {
  ...
  const { text, project_id, type } = body
...
Use today's date if meeting date is unclear. Match assignees and projects from context.`
      ...
      const meetingInfo = result.meeting ?? {}
      ...
      const topics = Array.isArray(meetingInfo.topics) ? meetingInfo.topics : []
      ...
      for (const topic of topics) {
        ...
        const actionItems = Array.isArray(topic.action_items) ? topic.action_items : []
