...
// The meetings API returns each meeting with its nested topics, and each topic
// with its notes + action items. The shared *Row types are flat, so compose them.
interface TopicWithChildren extends MeetingTopicRow {
  notes?: MeetingNoteRow[]
  actionItems?: MeetingActionItemRow[]
}
interface MeetingWithTopics extends MeetingRow {
...
const primary: CSSProperties = { background: 'var(--accent)', color: 'var(--accent-text)' }
const cancel: CSSProperties = { background: 'var(--surface-2)', color: 'var(--text-muted)' }
const danger: CSSProperties = { background: 'var(--pri-high-bg)', color: 'var(--red)' }
...
function Btn({ onClick, children, style = {} }: BtnProps) {
...
export default function MeetingsTab({ projectSlug: propProjectSlug }: MeetingsTabProps) {
  const { api, currentClient, allProjects } = useProject()
  const { getOptions } = useConfig()
  const topicColors = getOptions('topic_color')

  const projectSlug = propProjectSlug || currentClient?.slug

  const [meetings, setMeetings] = useState<MeetingWithTopics[]>([])
  const [loading, setLoading] = useState(true)
  const [openId, setOpenId] = useState<number | null>(null)
  const [showNewMeeting, setShowNewMeeting] = useState(false)
  const [newMeeting, setNewMeeting] = useState<NewMeeting>({ meeting_date: '', title: '', meeting_type: 'Weekly Sync', next_meeting: '', display_date: '' })
  const [addingTopicFor, setAddingTopicFor] = useState<number | null>(null)
  const [newTopic, setNewTopic] = useState<NewTopic>({ area: '', color: '' })
  ...
  const [newAction, setNewAction] = useState<NewAction>({ action_text: '', assignee_name: '' })
  const [editingMeeting, setEditingMeeting] = useState<MeetingWithTopics | null>(null)
  const [saving, setSaving] = useState(false)

  function load() {
    if (!projectSlug) return
    setLoading(true)
    api.get<MeetingWithTopics[]>(`/api/meetings?slug=${projectSlug}&all=1`)
  ...
  async function saveMeetingEdit(m: MeetingWithTopics) {
    setSaving(true)
  ...
  async function addTopic(meetingId: number) {
  ...
  async function addNote(topicId: number) {
  ...
  async function addAction(topicId: number) {
            ...
            {editingMeeting?.id === m.id ? (
              <div style={{ padding: 16 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                  <Field label="Date" type="date" value={editingMeeting.meeting_date} onChange={v => setEditingMeeting(p => p && ({ ...p, meeting_date: v }))} />
                  <Field label="Display Date" value={editingMeeting.display_date || ''} onChange={v => setEditingMeeting(p => p && ({ ...p, display_date: v }))} />
                  <Field label="Title" value={editingMeeting.title} onChange={v => setEditingMeeting(p => p && ({ ...p, title: v }))} />
                  <Field label="Meeting Type" value={editingMeeting.meeting_type || ''} onChange={v => setEditingMeeting(p => p && ({ ...p, meeting_type: v }))} />
                  <Field label="Next Meeting" value={editingMeeting.next_meeting || ''} onChange={v => setEditingMeeting(p => p && ({ ...p, next_meeting: v }))} />
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Project</label>
                    <select
                      value={editingMeeting.project_id}
                      onChange={e => setEditingMeeting(p => p && ({ ...p, project_id: Number(e.target.value) }))}
                      style={{ border: '1px solid var(--border)', borderRadius: 7, padding: '7px 10px', fontSize: 13, color: 'var(--text)', background: 'var(--surface)', width: '100%', fontFamily: 'inherit' }}
                    >
                      {allProjects.map(p => (
                ...
                {(m.topics || []).map(t => (
                  <div key={t.id} style={{ border: `1.5px solid ${t.color}30`, borderRadius: 10, marginBottom: 12, overflow: 'hidden' }}>
                    <div style={{ background: t.color + '14', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: t.color || undefined, display: 'inline-block', flexShrink: 0 }} />
                      ...
                      <div>
                        ...
                        {(t.notes || []).map((n, i) => (
                          <div key={n.id || i} style={{ display: 'flex', gap: 6, padding: '4px 0', fontSize: 12, color: 'var(--text)', alignItems: 'flex-start' }}>
                            <span style={{ color: t.color || undefined, flexShrink: 0 }}>•</span>
                            <span style={{ flex: 1 }}>{n.note_text || (n as unknown as string)}</span>
