...
import type { ProjectWithClient } from '../../context/ProjectContext'
import type { InvitationRow } from '../../../shared/types'

// A user as returned by /api/users, with client memberships from the join table.
interface AdminUser {
  id: string
  name: string | null
  email: string
...
function UserRow({ user, allProjects, onRefresh, currentUserId }: UserRowProps) {
  ...
  const savePassword = async () => {
    setPwdStatus('saving')
    const data = await api.post<{ ok?: boolean; error?: string }>(`/api/users/${user.id}/set-password`, { password: newPwd })
    if (data?.ok) {
      setPwdStatus('ok')
      setNewPwd('')
...
function InviteSection() {
  ...
  const loadInvites = () => {
    api.get<InvitationRow[]>('/api/invitations').then(data => {
  ...
  const createInvite = async () => {
    ...
    setGeneratedLink('')
    try {
      const data = await api.post<{ url?: string }>('/api/invitations', { slug: targetSlug, email: email.trim() || undefined })
      if (data.url) {
...
export default function UsersTab() {
  ...
  const loadUsers = () => {
    setLoading(true)
    api.get<AdminUser[]>('/api/users').then(data => {
