...
export interface ConfigContextValue {
  ...
  getValues: (category: string) => ConfigItem[]
  getOptions: (category: string) => string[]
  getColor: (category: string, value: string) => string | null
  reload: () => void
}
...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  const reload = useCallback(() => {
    if (!slug || !api) return
    api.get<ConfigMap>(`/api/config?slug=${slug}`)
      .then(setConfig)
  ...
  const getValues = (category: string): ConfigItem[] => {
  ...
  const getOptions = (category: string): string[] => getValues(category).map((item) => item.value)

  const getColor = (category: string, value: string): string | null =>
