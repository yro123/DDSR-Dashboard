import { requireProjectAccess, isAdmin, requireSession } from '../../lib/authz'
import { routeParam } from '../../lib/types'
import type { Ctx, Env } from '../../lib/types'
import type { TicketRequestRow, TaskRow } from '../../../shared/types'
...
const fetchTicket = (env: Env, id: string): Promise<TicketWithWorkflow | null> =>
...
export async function onRequestGet({ env, params, request }: Ctx): Promise<Response> {
  ...
  const ticket = await fetchTicket(env, id)
...
interface UpdateTicketBody {
  ...
  task_workflow_id?: number | string | null
  task_assignee_id?: number | string | null
  ...
  if (status) {
    ...
    if (status === 'Approved') {
      ...
      await env.ddsr_dashboard.prepare(`
        ...
        WHERE id = ?
      `).bind(taskId, reviewer_notes || null, session.user.name || session.user.email, now, now, id).run()
    ...
    if (status === 'Rejected') {
      ...
      `).bind(rejection_reason || null, reviewer_notes || null,
              session.user.name || session.user.email, now, now, id).run()
      ...
      await env.ddsr_dashboard.prepare(`
        ...
        SET status = 'Deferred', reviewer_notes = ?,
            reviewed_by = ?, reviewed_at = ?, updated_at = ?
        WHERE id = ?
      `).bind(reviewer_notes || null, session.user.name || session.user.email, now, now, id).run()
    } else if (status === 'In Review') {
      await env.ddsr_dashboard.prepare(`
        UPDATE ticket_requests
        SET status = 'In Review', reviewer_notes = ?, updated_at = ?
        WHERE id = ?
      `).bind(reviewer_notes || null, now, id).run()
    ...
    const updated = await fetchTicket(env, id)
...
export async function onRequestDelete({ env, params, request }: Ctx): Promise<Response> {
  ...
  await env.ddsr_dashboard.prepare('DELETE FROM ticket_requests WHERE id = ?').bind(id).run()
