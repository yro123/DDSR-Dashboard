...
import type { TicketRequestRow, WorkflowRow, PersonRow } from '../../shared/types'

/**
 * Ticket as returned by the list/detail endpoints — extends the raw row with
 * the joined display fields the API adds (workflow name, submitter name, etc.).
 */
interface Ticket extends TicketRequestRow {
...
import { formatTimestamp as fmtDate } from '../lib/dateUtils'

interface StatusColor { bg: string; col: string; border: string }
...
function PriorityDot({ priority, getColor }: { priority: string; getColor: GetColor }) {
...
function CategoryPill({ category, getColor }: { category: string; getColor: GetColor }) {
...
interface SubmitFormProps {
  ...
  people: PersonRow[]
  onSubmit: (form: SubmitFormState) => Promise<void> | void
  onCancel: () => void
  categories: string[]
  priorities: string[]
}

function SubmitForm({ workflows, onSubmit, onCancel, categories, priorities }: SubmitFormProps) {
...
interface ApprovePanelProps {
  ...
  people: PersonRow[]
  workflows: WorkflowRow[]
  onConfirm: (form: ApprovePanelForm) => Promise<void> | void
  onCancel: () => void
}

function ApprovePanel({ ticket, people, workflows, onConfirm, onCancel }: ApprovePanelProps) {
  const [form, setForm] = useState<ApprovePanelForm>({
    task_workflow_id: ticket.workflow_id ? String(ticket.workflow_id) : '',
    task_assignee_id: '',
    task_due_date: ticket.requested_due_date || '',
    reviewer_notes: '',
  })
  const set = (k: keyof ApprovePanelForm, v: string) => setForm(f => ({ ...f, [k]: v }))
  const [saving, setSaving] = useState(false)
...
function DeferPanel({ onConfirm, onCancel }: DeferPanelProps) {
...
type Panel = 'approve' | 'reject' | 'defer' | null
...
function TicketRow({ ticket, isAdmin, api, onUpdate, people, workflows, getColor }: TicketRowProps) {
  const [open, setOpen] = useState(false)
  const [panel, setPanel] = useState<Panel>(null)
  const [acting, setActing] = useState(false)

  const transition = async (status: string, extra: ApprovePanelForm | Record<string, unknown> = {}) => {
    setActing(true)
    // The PUT endpoint may return either the bare ticket or { ticket }.
    const res = await api.put<Ticket | { ticket: Ticket }>(`/api/tickets/${ticket.id}`, { status, ...extra })
    onUpdate(('ticket' in res ? res.ticket : res) as Ticket)
    setPanel(null)
    setActing(false)
  }
...
export default function Requests() {
  ...
  const { data: ticketsData = [], loading: ticketsLoading } = useQuery<Ticket[]>(
  ...
  const submitTicket = async (form: SubmitFormState) => {
    const res = await api.post<Ticket>('/api/tickets', { slug, ...form })
