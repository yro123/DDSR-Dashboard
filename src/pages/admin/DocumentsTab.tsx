...
import type { DocumentRow, WorkflowRow } from '../../../shared/types'
...
// A workflow (or the synthetic "Unassigned" bucket) with its documents attached.
interface DocGroup {
  name: string
  docs: DocumentRow[]
}
...
export default function DocumentsTab({ projectSlug: propProjectSlug }: DocumentsTabProps) {
  ...
  const [newDoc, setNewDoc] = useState<NewDoc>({ name: '', url: '', doc_type: 'Process Doc', workflow_id: '' })
  const [saving, setSaving] = useState(false)

  function reload() {
    Promise.all([
      api.get<DocumentRow[]>(`/api/documents?slug=${projectSlug}`),
      api.get<WorkflowRow[]>(`/api/workflows?slug=${projectSlug}`),
    ]).then(([d, w]) => {
  ...
  const byWorkflow: DocGroup[] = workflows.map(w => ({
    name: w.name,
