 ...
 */
export async function buildDomainMap(db) {
 ...
 */
export function findMatch(snapshot, domainMap) {
 ...
 * @param {string} sort  urgency | criticality | resolution | received
 * @param {string} dir   asc | desc
 */
export function orderByClause(sort, dir) {
  const direction = String(dir).toLowerCase() === 'asc' ? 'ASC' : 'DESC'
  const ratingCase = col => `CASE ${col} WHEN 'High' THEN 3 WHEN 'Medium' THEN 2 WHEN 'Low' THEN 1 ELSE 0 END`
