...
export default function ClientsTab() {
  ...
  const [newClient, setNewClient] = useState<NewClient>({ name: '', display_name: '', slug: '', email_domain: '' })
  const [addingProjectFor, setAddingProjectFor] = useState<number | null>(null)
  const [newProject, setNewProject] = useState<NewProject>({ name: '', subtitle: '', slug: '', go_live_date: '' })
  ...
  async function createClient() {
    ...
    try {
      ...
      showToast((err as Error)?.message || 'Failed to create client', 'error')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function saveClientEdit() {
    ...
    try {
      ...
      showToast((err as Error)?.message || 'Failed to save client', 'error')
    } finally {
      setSaving(false)
    }
  }

  async function setClientActive(client: ClientWithProjects, is_active: number) {
    ...
    try {
      ...
      showToast((err as Error)?.message || 'Failed to update client', 'error')
    } finally {
      setSaving(false)
    }
  }

  async function createProject(clientId: number) {
            ...
            {editingClient?.id === client.id ? (
              <div style={{ padding: 16 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
                  <div><label style={labelStyle}>Internal Name</label><input value={editingClient.name} onChange={e => setEditingClient(p => p && ({ ...p, name: e.target.value }))} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Display Name</label><input value={editingClient.display_name} onChange={e => setEditingClient(p => p && ({ ...p, display_name: e.target.value }))} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Slug (caution)</label><input value={editingClient.slug} onChange={e => setEditingClient(p => p && ({ ...p, slug: e.target.value as ClientWithProjects['slug'] }))} style={inputStyle} /></div>
                  <div><label style={labelStyle}>Email Domain</label><input value={editingClient.email_domain || ''} onChange={e => setEditingClient(p => p && ({ ...p, email_domain: e.target.value }))} placeholder="acmecorp.com" style={inputStyle} /></div>
                </div>
